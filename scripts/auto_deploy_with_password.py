...
def connect_with_password():
    """使用密码连接到云服务器"""
    print(f"[1/7] 使用密码连接到云服务器 {SERVER_HOST}...")
...
def run_deploy_script(ssh):
    ...
    try:
        print("  执行: bash ../scripts/cloud_auto_deploy.sh")
...
def main():
    ...
    # 连接到云服务器
    ssh = connect_with_password()

    # 上传部署包
    remote_filepath = upload_package(ssh, DEPLOY_PACKAGE, "/tmp/")
