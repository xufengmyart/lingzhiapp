...
test_user_login() {
    ...
    if echo "$response" | grep -q '"success":true'; then
        ...
        USER_ID=$(echo "$response" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['user']['id'])")
        test_pass "登录成功，用户ID: $USER_ID"
        return 0
    else
        test_fail "登录失败"
        echo "$response"
...
test_user_info() {
    ...
    if echo "$response" | grep -q '"success":true'; then
        ...
        if echo "$response" | grep -q '"referrer"'; then
            ...
            # 显示推荐人信息
            local referrer_info=$(echo "$response" | python3 -c "
import sys, json
data = json.load(sys.stdin)
referrer = data['data']['user'].get('referrer')
if referrer and referrer.get('id'):
    print(f'  推荐人ID: {referrer.get(\"id\")}')
    print(f'  推荐人: {referrer.get(\"username\", \"N/A\")}')
else:
    print('  无推荐人')
")
            echo "$referrer_info"
...
test_token_validation() {
    ...
    if [ $success_count -eq 3 ]; then
        test_pass "Token验证成功（3/3）"
        return 0
    else
        test_fail "Token验证失败（${success_count}/3）"
        return 1
    fi
}
...
print_summary() {
    ...
    if [ $FAILED_TESTS -eq 0 ]; then
        ...
        echo "2. 检查应用日志: ssh user@meiyueart.com 'tail -f /var/log/flask_backend.log'"
        echo "3. 监控API性能和错误率"
        return 0
    else
        ...
        echo ""
        echo "建议操作:"
        echo "1. 查看服务日志: ssh user@meiyueart.com 'tail -100 /var/log/flask_backend.log'"
        echo "2. 检查错误信息"
        echo "3. 如需回滚，执行: ssh user@meiyueart.com 'cp /path/to/backup/user_system.py.YYYYMMDD_HHMMSS /path/to/admin-backend/routes/user_system.py'"
        return 1
    fi
}
