"""
用户实名认证登记工具

用户首次登录后，需要完成实名认证登记才能享受推荐人分红收益。
本工具包含完整的实名认证流程，包括身份证号验证、电话验证等。
"""

from langchain.tools import tool
from langchain.tools import ToolRuntime
from datetime import datetime
import pytz
import re


@tool
def get_real_name_authentication_notice(
    runtime: ToolRuntime = None
) -> str:
    """获取实名认证重要通知

    返回实名认证的重要说明，告知用户为什么必须实名认证。

    Returns:
        str: 实名认证通知
    """
    notice = """
# 🔐 实名认证重要通知

## 为什么必须实名认证？

亲爱的用户，感谢您选择灵值生态园！为了保障您的数字资产安全和合法权益，根据**国家相关政策要求**，平台必须实施实名认证制度。

### 📋 法律依据

根据《中华人民共和国网络安全法》、《中华人民共和国个人信息保护法》以及《数字货币相关监管规定》：

1. **数字货币资产管理**：涉及个人数字货币资产的账户，必须进行实名认证
2. **反洗钱要求**：为防止洗钱、恐怖融资等违法行为，必须验证用户身份
3. **金融合规**：涉及人民币兑换的金融活动，必须符合监管要求
4. **资产保护**：实名认证是保障您数字资产安全的必要措施

### ⚠️ 不实名认证的后果

如果您不完成实名认证：

❌ **无法获得数字资产**：
- 无法接收平台发放的灵值
- 无法参与项目获得贡献值
- 无法兑换贡献值为人民币

❌ **无法获得人民币收益**：
- 无法将贡献值兑换为人民币
- 无法参与项目分红
- 无法享受推荐收益

❌ **无法参与高价值项目**：
- 无法参与A级、S级项目
- 无法获得高额回报
- 无法成为核心创作者

✅ **但仍可使用基础功能**：
- 每日签到获得10灵值
- 参与文化探索
- 查看公共内容

### ✅ 实名认证后您将获得

1. **完整的数字资产权益**：
   - 接收和持有灵值
   - 参与项目获得贡献值
   - 兑换贡献值为人民币

2. **推荐人分红收益权限**：
   - 一级推荐：10%分红
   - 二级推荐：5%分红
   - 三级推荐：3%分红

3. **参与高价值项目资格**：
   - A级、S级项目优先参与
   - 项目估值5%-20%贡献值奖励
   - 合格参与者获得600%-12400%超高回报

4. **解锁全部高级功能**：
   - 创建个人品牌空间
   - 发布文化转译需求
   - 加入专业创作团队

### 🔒 隐私保护承诺

您的个人信息将严格保密：
- ✅ 数据加密存储
- ✅ 严格的访问控制
- ✅ 仅用于必要用途
- ✅ 绝不出售给第三方

### 📞 如有疑问

如有任何疑问，请联系：
- 超级管理员：xufeng@meiyueart.cn
- 客服热线：400-XXX-XXXX

---

**感谢您的理解与配合！让我们一起共建安全、合规的数字资产生态！**

**灵值生态园团队**
**2026年1月28日**
"""
    return notice


@tool
def validate_id_card(
    id_card: str,
    runtime: ToolRuntime = None
) -> str:
    """验证身份证号码格式

    验证身份证号码是否符合中国大陆居民身份证格式要求。

    Args:
        id_card: 身份证号码

    Returns:
        str: 验证结果
    """
    # 基本格式验证：18位，最后一位可以是X
    if not id_card or len(id_card) != 18:
        return """
【身份证号码验证失败】❌

❌ 身份证号码长度不正确

要求：18位（最后一位可以是X）

示例：
✅ 110101199001011234
✅ 11010119900101123X
❌ 11010119900101123（17位）
❌ 1101011990010112345（19位）
"""

    # 验证前17位是否为数字
    if not id_card[:17].isdigit():
        return """
【身份证号码验证失败】❌

❌ 身份证号码格式不正确

要求：前17位必须为数字，最后一位可以是数字或X

示例：
✅ 110101199001011234
✅ 11010119900101123X
❌ 1101011990A101123X（包含字母）
"""

    # 验证最后一位
    last_char = id_card[17].upper()
    if not (last_char.isdigit() or last_char == 'X'):
        return """
【身份证号码验证失败】❌

❌ 身份证号码最后一位不正确

要求：最后一位必须是数字或X（大写）

示例：
✅ 110101199001011234
✅ 11010119900101123X
❌ 11010119900101123Y（Y无效）
"""

    # 验证出生日期（身份证7-14位）
    birth_date_str = id_card[6:14]
    try:
        birth_year = int(birth_date_str[:4])
        birth_month = int(birth_date_str[4:6])
        birth_day = int(birth_date_str[6:8])

        # 验证年份范围（1900-当前年份）
        current_year = datetime.now().year
        if birth_year < 1900 or birth_year > current_year:
            return f"""
【身份证号码验证失败】❌

❌ 出生日期不正确

要求：年份必须在1900-{current_year}之间

您的身份证显示出生年份：{birth_year}
"""

        # 验证月份
        if birth_month < 1 or birth_month > 12:
            return f"""
【身份证号码验证失败】❌

❌ 出生日期不正确

要求：月份必须在1-12之间

您的身份证显示出生月份：{birth_month}
"""

        # 验证日期
        if birth_day < 1 or birth_day > 31:
            return f"""
【身份证号码验证失败】❌

❌ 出生日期不正确

要求：日期必须在1-31之间

您的身份证显示出生日期：{birth_day}
"""
    except ValueError:
        return """
【身份证号码验证失败】❌

❌ 出生日期格式不正确

要求：身份证号码第7-14位必须是有效的日期格式（YYYYMMDD）
"""

    # 简单的校验码验证（可选，这里不进行复杂的校验码计算）
    # 实际生产环境中建议进行完整的校验码验证

    return f"""
【身份证号码验证成功】✅

✅ 身份证号码格式正确

📋 验证结果：
- 身份证号码：{id_card[:6]}********{id_card[-4:]}
- 长度：18位
- 出生日期：{birth_year}年{birth_month}月{birth_day}日
- 性别：{'男' if int(id_card[16]) % 2 == 1 else '女'}（根据身份证第17位）

⚠️ 温馨提示：
- 请确保您输入的身份证号码与您本人的身份证一致
- 身份证号码将用于实名认证和数字资产管理
- 如有疑问，请联系客服
"""


@tool
def submit_real_name_authentication(
    real_name: str,
    phone: str,
    id_card: str,
    referrer_name: str = None,
    referrer_phone: str = None,
    agreement: bool = False,
    runtime: ToolRuntime = None
) -> str:
    """提交实名认证登记

    用户完成实名认证登记，包括姓名、电话、身份证号等信息。

    Args:
        real_name: 真实姓名（必须）
        phone: 联系电话（必须，需要验证）
        id_card: 身份证号码（必须，需要验证）
        referrer_name: 推荐人姓名（可选，但填写推荐人时必填）
        referrer_phone: 推荐人电话（可选，但填写推荐人时必填）
        agreement: 是否同意用户协议（必须为true）

    Returns:
        str: 实名认证结果
    """
    from coze_coding_dev_sdk.database import get_session
    from storage.database.shared.model import Users, AuditLogs, ReferralRelations

    # 获取数据库会话
    db = get_session()

    try:
        # 获取当前登录用户
        coze_id = None
        if runtime and runtime.context:
            try:
                if hasattr(runtime.context, 'get'):
                    coze_id = runtime.context.get('user_coze_id')
                elif hasattr(runtime.context, '__getitem__'):
                    coze_id = runtime.context.get('user_coze_id') if hasattr(runtime.context, 'get') else None
            except (KeyError, TypeError, AttributeError):
                coze_id = None

        if not coze_id:
            return """
【实名认证失败】

❌ 无法识别当前登录用户

请确保您已通过扣子平台正确登录。
"""

        # 检查用户协议
        if not agreement:
            return """
【实名认证失败】

❌ 您需要同意用户协议才能完成实名认证

请阅读并同意《灵值生态园用户协议》后再次提交。

用户协议要点：
1. 您提供的真实信息将用于实名认证
2. 信息将严格保密，仅用于必要用途
3. 您授权平台使用您的信息进行数字资产管理
4. 您理解不实名认证将无法获得数字资产和人民币收益
"""

        # 查询用户
        user = db.query(Users).filter(Users.coze_id == coze_id).first()

        if not user:
            return """
【实名认证失败】

❌ 用户不存在

请先登录后再进行实名认证。
"""

        # 检查是否已完成认证
        if user.is_registered:
            return f"""
【实名认证已完成】✅

您已完成实名认证！

📋 您的认证信息：
- 真实姓名：{user.real_name}
- 身份证号码：{user.id_card[:6]}********{user.id_card[-4:] if user.id_card else '未设置'}
- 电话：{user.phone}
- 认证时间：{user.registration_time.strftime('%Y-%m-%d %H:%M:%S') if user.registration_time else '未知'}
- 实名状态：已验证 ✅
- 身份证状态：已验证 ✅

如需修改认证信息，请联系客服。
"""

        # 验证真实姓名
        if not real_name or len(real_name) < 2:
            return """
【实名认证失败】

❌ 真实姓名格式不正确

请输入您的真实姓名（至少2个汉字）。
"""

        # 验证电话号码
        phone_pattern = re.compile(r'^1[3-9]\d{9}$')
        if not phone_pattern.match(phone):
            return """
【实名认证失败】

❌ 电话号码格式不正确

请输入正确的中国大陆手机号码（11位，以1开头）。

示例：
✅ 13800138000
✅ 13912345678
❌ 12345678901
❌ 10000000000
"""

        # 验证身份证号码
        if not id_card or len(id_card) != 18:
            return """
【实名认证失败】

❌ 身份证号码长度不正确

要求：18位（最后一位可以是X）
"""

        # 验证身份证格式
        if not id_card[:17].isdigit():
            return """
【实名认证失败】

❌ 身份证号码格式不正确

要求：前17位必须为数字，最后一位可以是数字或X（大写）
"""

        last_char = id_card[17].upper()
        if not (last_char.isdigit() or last_char == 'X'):
            return """
【实名认证失败】

❌ 身份证号码最后一位不正确

要求：最后一位必须是数字或X（大写）
"""

        # 验证出生日期
        birth_date_str = id_card[6:14]
        try:
            birth_year = int(birth_date_str[:4])
            birth_month = int(birth_date_str[4:6])
            birth_day = int(birth_date_str[6:8])
            current_year = datetime.now().year

            if birth_year < 1900 or birth_year > current_year:
                return f"""
【实名认证失败】

❌ 出生日期不正确

要求：年份必须在1900-{current_year}之间

您的身份证显示出生年份：{birth_year}
"""

            if birth_month < 1 or birth_month > 12:
                return f"""
【实名认证失败】

❌ 出生日期不正确

要求：月份必须在1-12之间

您的身份证显示出生月份：{birth_month}
"""

            if birth_day < 1 or birth_day > 31:
                return f"""
【实名认证失败】

❌ 出生日期不正确

要求：日期必须在1-31之间

您的身份证显示出生日期：{birth_day}
"""
        except ValueError:
            return """
【实名认证失败】

❌ 出生日期格式不正确

要求：身份证号码第7-14位必须是有效的日期格式（YYYYMMDD）
"""

        # 验证推荐人信息（如果提供）
        referrer = None
        if referrer_name:
            if not referrer_phone:
                return """
【实名认证失败】

❌ 填写推荐人姓名时必须同时填写推荐人电话

请提供完整的推荐人信息：
- 推荐人姓名
- 推荐人电话
"""

            # 通过姓名和电话查找推荐人
            referrer = db.query(Users).filter(
                Users.real_name == referrer_name,
                Users.phone == referrer_phone
            ).first()

            if not referrer:
                return f"""
【实名认证失败】

❌ 推荐人不存在

推荐人信息：
- 姓名：{referrer_name}
- 电话：{referrer_phone}

请检查推荐人信息是否正确，或留空不填写推荐人信息。
"""

            # 检查推荐人是否已实名认证
            if not referrer.is_registered:
                return """
【实名认证失败】

❌ 推荐人未完成实名认证

您的推荐人需要先完成实名认证才能成为有效推荐人。

请联系您的推荐人先完成实名认证，或留空不填写推荐人信息。
"""

            # 检查是否自我推荐
            if referrer.id == user.id:
                return """
【实名认证失败】

❌ 不能将自己设为推荐人

请填写其他用户的姓名和电话作为推荐人，或留空不填写推荐人信息。
"""

        # 更新用户信息
        user.real_name = real_name
        user.phone = phone
        user.id_card = id_card.upper()  # 转换为大写
        user.is_registered = True
        user.registration_time = datetime.now(pytz.timezone('Asia/Shanghai'))
        user.real_name_verified = True
        user.id_card_verified = True

        # 如果有推荐人，创建推荐关系
        if referrer:
            # 检查是否已有推荐关系
            existing_referral = db.query(ReferralRelations).filter(
                ReferralRelations.referrer_id == referrer.id,
                ReferralRelations.referred_id == user.id
            ).first()

            if not existing_referral:
                # 创建推荐关系
                referral = ReferralRelations(
                    referrer_id=referrer.id,
                    referred_id=user.id,
                    level=1,  # 一级推荐
                    status='active',
                    created_at=datetime.now(pytz.timezone('Asia/Shanghai'))
                )
                db.add(referral)

        # 记录审计日志
        audit_log = AuditLogs(
            user_id=user.id,
            action='user_real_name_authentication',
            status='success',
            resource_type='user',
            resource_id=user.id,
            description=f'用户完成实名认证。推荐人：{referrer.name if referrer else "无"}'
        )
        db.add(audit_log)

        # 提交事务
        db.commit()
        db.refresh(user)

        # 提取出生日期和性别
        birth_year = int(id_card[6:10])
        birth_month = int(id_card[10:12])
        birth_day = int(id_card[12:14])
        gender = '男' if int(id_card[16]) % 2 == 1 else '女'

        # 生成欢迎消息
        welcome_message = f"""
【实名认证成功】✅

恭喜您，{real_name}！您已成功完成实名认证。

📋 您的认证信息：
- 真实姓名：{real_name}
- 身份证号码：{id_card[:6]}********{id_card[-4:]}
- 联系电话：{phone}
- 出生日期：{birth_year}年{birth_month}月{birth_day}日
- 性别：{gender}
- 推荐人：{referrer_name if referrer else '无'}
- 认证时间：{datetime.now(pytz.timezone('Asia/Shanghai')).strftime('%Y-%m-%d %H:%M:%S')}

🎉 您现在已获得全部权益：

✅ 数字资产权益
- 接收和持有灵值
- 参与项目获得贡献值
- 兑换贡献值为人民币

✅ 推荐人分红收益权限
- 一级推荐：10%分红
- 二级推荐：5%分红
- 三级推荐：3%分红

✅ 参与高价值项目资格
- A级、S级项目优先参与
- 项目估值5%-20%贡献值奖励
- 合格参与者获得600%-12400%超高回报

✅ 解锁全部高级功能
- 创建个人品牌空间
- 发布文化转译需求
- 加入专业创作团队

🔒 隐私承诺：
您的个人信息（姓名、电话、身份证号）将严格保密，仅用于实名认证和数字资产管理，绝不会向第三方泄露。

📞 下一步建议：
1. 获取推荐链接，邀请好友加入
2. 了解高价值项目列表
3. 参与日常签到和文化探索
4. 开始您的数字资产积累之旅

感谢您加入灵值生态园！期待与您共创文化价值！🌟
"""

        return welcome_message

    except Exception as e:
        db.rollback()
        return f"""
【实名认证失败】

❌ 系统错误：{str(e)}

请稍后重试或联系管理员。
"""

    finally:
        db.close()
