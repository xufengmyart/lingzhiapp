...
const LoginFull = () => {
  ...
  // 如果用户已登录且不在登录页面，直接跳转到dashboard
  useEffect(() => {
    const currentPath = window.location.pathname
    const isLoginPage = currentPath === '/' || currentPath === '/login-full'
    ...
    if (user && !isLoginPage) {
      console.log('[LoginFull] 用户已登录，跳转到dashboard')
  ...
  const handleWechatLogin = async () => {
    try {
      ...
      // 修复：移除重复的/api前缀，使用正确的微信开放平台OAuth2.0接口
      const response = await fetch(`${apiBase}/wechat/oauth/authorize`)

      // 处理503错误（服务不可用）
      if (response.status === 503) {
        setError('微信扫码登录暂时不可用，请使用手机号登录')
        setLoading(false)
        return
      }
      ...
      if (data.success && data.data.authUrl) { // 注意后端返回的是authUrl而不是auth_url
        // 保存state用于验证回调
        sessionStorage.setItem('wechat_state', data.data.state)
        // 跳转到微信授权页面
        window.location.href = data.data.authUrl
      } else {
        // 检查是否有友好的替代方案提示
        const errorMessage = data.data?.alternative ||
                           data.data?.hint ||
                           data.message ||
                           '微信登录配置错误，请联系管理员'
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-5">
            ...
            <div className="space-y-3 pt-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full relative py-3 px-4 bg-gradient-to-r ${vrStyle.buttonBg} text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 overflow-hidden group ${loading ? 'opacity-70' : ''}`}
              >
                <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                {loading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <Lock className="w-5 h-5" />}
                <span className="ml-2 text-sm">账号密码登录</span>
              </button>
              ...
              <button
                ...
                className="relative w-full py-3 px-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 overflow-hidden group"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="flex items-center justify-center">
                  <MessageCircle className="w-5 h-5" />
                  <div className="ml-2 flex flex-col items-start">
                    <span className="text-sm font-medium">微信智能登录</span>
                    <span className="text-xs opacity-80">自动识别·一键注册/登录</span>
                  </div>
                  <Sparkles className="w-4 h-4 ml-auto opacity-80" />
                </div>
              </button>
            </div>
          </form>
