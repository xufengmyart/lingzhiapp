...
const Recharge = () => {
  ...
  const loadRechargeTiers = async () => {
    try {
      ...
      const response = await fetch(`${apiBase}/api/recharge/tiers`)
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const apiBase = import.meta.env.VITE_API_BASE_URL || ''
      const response = await fetch(`${apiBase}/api/recharge/create-order`, {
        method: 'POST',
        headers: {
  ...
  const handleCompletePayment = async (orderNo: string) => {
    try {
      const token = localStorage.getItem('token')
      const apiBase = import.meta.env.VITE_API_BASE_URL || ''
      const response = await fetch(`${apiBase}/api/recharge/complete-payment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      if (data.success) {
        ...
        // 刷新用户信息
        const userResponse = await fetch(`${apiBase}/api/user/info`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        })
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <div className="text-3xl font-bold">{user?.totalLingzhi || 0}</div>
              <div className="text-sm opacity-80">等值：¥{((user?.totalLingzhi || 0) * 0.1).toFixed(2)}</div>
            </div>
          </div>
