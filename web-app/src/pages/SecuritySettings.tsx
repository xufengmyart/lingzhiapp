...
const SecuritySettings = () => {
  ...
  const loadDevices = async () => {
    try {
      ...
      const response = await fetch(`${apiBase}/api/user/devices`, {
        headers: {
  ...
  const loadSettings = async () => {
    try {
      const token = localStorage.getItem('token')
      const apiBase = import.meta.env.VITE_API_BASE_URL || ''
      const response = await fetch(`${apiBase}/api/user/security/settings`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      })
  ...
  const handleRemoveDevice = async (deviceId: string) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const apiBase = import.meta.env.VITE_API_BASE_URL || ''
      const response = await fetch(`${apiBase}/api/user/devices/${deviceId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleRevokeAll = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const apiBase = import.meta.env.VITE_API_BASE_URL || ''
      const response = await fetch(`${apiBase}/api/user/devices/revoke-all`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
