...
const KnowledgeManagement = () => {
  ...
  const fetchKnowledgeBases = async () => {
    try {
      setLoading(true)
      const response = await api.get('/api/admin/knowledge-bases')
  ...
  const fetchDocuments = async (kbId: number) => {
    try {
      const response = await api.get(`/api/admin/knowledge-bases/${kbId}`)
      if (response.data.success) {
  ...
  const handleDelete = async (kbId: number) => {
    ...
    try {
      const response = await api.delete(`/api/admin/knowledge-bases/${kbId}`)
      if (response.data.success) {
        setMessage('知识库删除成功')
  ...
  const handleSubmitKB = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/api/admin/knowledge-bases', formData)
      if (response.data.success) {
        setMessage('知识库创建成功')
        setIsCreating(false)
        fetchKnowledgeBases()
  ...
  const handleUpdateKB = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.put(`/api/admin/knowledge-bases/${selectedKB.id}`, formData)
      if (response.data.success) {
        setMessage('知识库更新成功')
        fetchKnowledgeBases()
  ...
  const handleAddDocument = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post(`/api/admin/knowledge-bases/${selectedKB.id}/documents`, docForm)
      if (response.data.success) {
        setMessage('文档添加成功')
  ...
  const handleDeleteDocument = async (docId: number) => {
    ...
    try {
      const response = await api.delete(`/api/admin/knowledge-bases/${selectedKB.id}/documents/${docId}`)
      if (response.data.success) {
        setMessage('文档删除成功')
        fetchDocuments(selectedKB.id)
        fetchKnowledgeBases()
