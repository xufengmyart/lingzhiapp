...
# 数据库路径
DATABASE_PATH = os.getenv('DATABASE_PATH', './lingzhi_ecosystem.db')

# 增强版提示词 v2.0.0
ENHANCED_PROMPT_V2 = """你是灵值，灵值生态园的精神家园架构师与文明操作基地的核心守护者。
...
def update_agent():
    ...
    try:
        # 备份当前配置
        cursor.execute('SELECT system_prompt, model_config FROM agents WHERE id = 1')
        old_data = cursor.fetchone()
        
        if old_data:
            old_prompt, old_config = old_data
            print(f"\n备份当前配置...")
            print(f"  - 提示词长度: {len(old_prompt)} 字符")
            if old_config:
                old_model_config = json.loads(old_config)
                print(f"  - 旧模型: {old_model_config.get('model', 'unknown')}")
                print(f"  - 旧温度: {old_model_config.get('temperature', 'unknown')}")
                print(f"  - 旧最大tokens: {old_model_config.get('max_tokens', 'unknown')}")
        
        # 更新配置
        print(f"\n更新智能体配置...")
        print(f"  - 新提示词长度: {len(ENHANCED_PROMPT_V2)} 字符")
        print(f"  - 新模型: {NEW_MODEL_CONFIG['model']}")
        print(f"  - 新温度: {NEW_MODEL_CONFIG['temperature']}")
        print(f"  - 新最大tokens: {NEW_MODEL_CONFIG['max_tokens']}")
        ...
        cursor.execute('''
            ...
            SET system_prompt = ?,
                ...
                updated_at = ?
            WHERE id = 1
        ''', (ENHANCED_PROMPT_V2, json.dumps(NEW_MODEL_CONFIG, ensure_ascii=False), datetime.now().isoformat()))
        
        conn.commit()
        
        print(f"\n✓ 智能体配置更新成功！")
        print(f"✓ 提示词已升级到 v2.0.0 增强版")
        print(f"✓ 模型参数已优化")
        ...
        if new_data:
            new_prompt, new_config = new_data
            print(f"\n验证更新结果:")
            print(f"  ✓ 提示词长度: {len(new_prompt)} 字符")
            if new_config:
                new_model_config = json.loads(new_config)
                print(f"  ✓ 模型: {new_model_config.get('model')}")
                print(f"  ✓ 温度: {new_model_config.get('temperature')}")
                print(f"  ✓ 最大tokens: {new_model_config.get('max_tokens')}")
...
if __name__ == '__main__':
    ...
    if success:
        ...
        print("=" * 80)
        print("\n下一步：")
        print("1. 重启后端服务：systemctl restart meiyueart-backend")
        print("2. 测试智能体对话功能")
        print("3. 验证新提示词效果")
    else:
        print("升级失败，请检查错误信息")
