...
def seed_default_data():
    ...
    try:
        ...
        # CMO管理员权限
        cmo_role = db.query(Role).filter_by(name_en="cmo_admin").first()
        cmo_permissions = [
            "user:create", "user:modify", "user:view", "user:reset_password",
            "role:create", "role:modify", "role:view", "role:assign_permission",
            "permission:grant", "permission:revoke", "permission:view",
            ...
            "data:view", "data:modify", "data:export",
            "approval:submit", "approval:approve", "approval:view",
            "marketing:create", "marketing:modify", "marketing:view",
            "document:create", "document:modify", "document:view",
            "log:view", "log:export"
        ]
        cmo_role.permissions = [p for p in permissions if p.code in cmo_permissions]

        # COO管理员权限
        coo_role = db.query(Role).filter_by(name_en="coo_admin").first()
        coo_permissions = [
            ...
            "data:view", "data:modify", "data:export",
            "approval:submit", "approval:approve", "approval:view",
            "operation:manage", "operation:view",
            "document:create", "document:modify", "document:view",
            "log:view", "log:export"
        ]
        coo_role.permissions = [p for p in permissions if p.code in coo_permissions]

        # CFO管理员权限
        cfo_role = db.query(Role).filter_by(name_en="cfo_admin").first()
        cfo_permissions = [
            "user:create", "user:modify", "user:view",
            "role:view",
            "permission:view",
            ...
            "data:view", "data:modify", "data:export",
            "approval:submit", "approval:approve", "approval:view",
            "finance:view", "finance:modify", "finance:approve_budget",
            "document:create", "document:modify", "document:view",
            "log:view", "log:export"
        ]
        cfo_role.permissions = [p for p in permissions if p.code in cfo_permissions]
        ...
        print("初始账号信息：")
        print("="*50)
        print("账号：xufeng@meiyue.com")
        print("密码：Meiyue@2026")
